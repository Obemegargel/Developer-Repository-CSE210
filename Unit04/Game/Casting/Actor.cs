...
namespace Unit04.Game.Casting
{
    ...
    public class Actor
    {
        ...
        public void MoveNext(int maxX, int maxY)
        {
            ...
            int y = maxY - 20;//((_position.GetY() + _velocity.GetY()) + maxY) % maxY; //This makes the actor only go side to side. Might conflict with artifacts
        ...
        /// <param name="maxX">The maximum x value.</param>
        /// <param name="maxY">The maximum y value.</param>
        public void MoveArtifactNext(int maxX, int maxY)
        {
            int x = ((_position.GetX() + _velocity.GetX()) + maxX) % maxX;
            int y = ((_position.GetY() + _velocity.GetY())+1 + maxY) % maxY; //((_position.GetY() + _velocity.GetY()) + maxY) % maxY;//This makes the actor only go side to side. Might conflict with artifacts
            _position = new Point(x, y);
        }
